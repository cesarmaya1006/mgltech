...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="#">Historias clínicas ocupacionales</a></li>
@endsection
@section('titulo_card')
<img src="{{asset('imagenes/sistema/5.png')}}" class="img-fluid" alt="..." style="max-height: 35px; width: auto;"> Historias clínicas ocupacionales
@endsection
                    ...
                    @else
                        @if ($grupos->count()==1)
                            <select id="empresa_id" class="form-control form-control-sm"
                                    data_url="{{ route('histclinicasocup.getCargarEmpleadosEmpresa') }}">
...
<div class="row">
    <div class="col-12">
        <input type="hidden" id="datosUpLaod"
                data_soportes="{{route('histclinicasocup.getSoportes')}}"
                data_url="{{route('histclinicasocup.setSoportes')}}"
                ruta_soporte="{{asset('documentos/soportes') . '/'}}"
                tabla ="tablaSoportes"
                tbody ="tbody_Soportes"
                tipoSoporte ="HistClinicasOcup"
                titulo ="Ingrese la historia clínica ocupacional"
                aviso ="La historia clínica ocupacional">
        <input type="hidden" id="borrarSoportes" data_url="{{route('histclinicasocup.eliminarSoportes')}}">
        @csrf
    </div>
</div>
<!-- Modal addManual -->
