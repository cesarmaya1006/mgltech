...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="#">Capacitaciones y certificaciones</a></li>
@endsection
@section('titulo_card')
<img src="{{asset('imagenes/sistema/11.png')}}" class="img-fluid" alt="..." style="max-height: 35px; width: auto;"> Capacitaciones y certificaciones
@endsection
                    ...
                    @else
                        @if ($grupos->count()==1)
                            <select id="empresa_id" class="form-control form-control-sm"
                                    data_url="{{ route('capacitacion.getCargarEmpleadosEmpresa') }}">
...
<div class="row">
    <div class="col-12">
        <input type="hidden" id="datosUpLaod"
                data_soportes="{{route('capacitacion.getSoportes')}}"
                data_url="{{route('capacitacion.setSoportes')}}"
                ruta_soporte="{{asset('documentos/soportes') . '/'}}"
                tabla ="tablaSoportes"
                tbody ="tbody_Soportes"
                tipoSoporte ="Capacitacion"
                titulo ="Ingrese el soporte de capacitaciones de Retiro"
                aviso ="El soporte de capacitaciones de Retiro">
        <input type="hidden" id="borrarSoportes" data_url="{{route('capacitacion.eliminarSoportes')}}">
        @csrf
    </div>
</div>
<!-- Modal addManual -->
