...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="#">Evaluaciones de desempeño</a></li>
@endsection
@section('titulo_card')
<img src="{{asset('imagenes/sistema/8.png')}}" class="img-fluid" alt="..." style="max-height: 35px; width: auto;"> Evaluaciones de desempeño
@endsection
                    ...
                    @else
                        @if ($grupos->count()==1)
                            <select id="empresa_id" class="form-control form-control-sm"
                                    data_url="{{ route('evaluacion_desemp.getCargarEmpleadosEmpresa') }}">
...
<div class="row">
    <div class="col-12">
        <input type="hidden" id="datosUpLaod"
                data_soportes="{{route('evaluacion_desemp.getSoportes')}}"
                data_url="{{route('evaluacion_desemp.setSoportes')}}"
                ruta_soporte="{{asset('documentos/soportes') . '/'}}"
                tabla ="tablaSoportes"
                tbody ="tbody_Soportes"
                tipoSoporte ="EvaluacionDesemp"
                titulo ="Ingrese la evaluación de desempeño clínica ocupacional"
                aviso ="La evaluación de desempeño">
        <input type="hidden" id="borrarSoportes" data_url="{{route('evaluacion_desemp.eliminarSoportes')}}">
        @csrf
    </div>
</div>
<!-- Modal addManual -->
