...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="#">Entrega de dotación</a></li>
@endsection
@section('titulo_card')
<img src="{{asset('imagenes/sistema/6.png')}}" class="img-fluid" alt="..." style="max-height: 35px; width: auto;"> Entrega de dotación, elementos de trabajo y de protección
@endsection
                    ...
                    @else
                        @if ($grupos->count()==1)
                            <select id="empresa_id" class="form-control form-control-sm"
                                    data_url="{{ route('dotaciones.getCargarEmpleadosEmpresa') }}">
...
<div class="row">
    <div class="col-12">
        <input type="hidden" id="datosUpLaod"
                data_soportes="{{route('dotaciones.getSoportes')}}"
                data_url="{{route('dotaciones.setSoportes')}}"
                ruta_soporte="{{asset('documentos/soportes') . '/'}}"
                tabla ="tablaSoportes"
                tbody ="tbody_Soportes"
                tipoSoporte ="Dotaciones"
                titulo ="Ingrese la entrega de dotación"
                aviso ="La entrega de dotación">
        <input type="hidden" id="borrarSoportes" data_url="{{route('dotaciones.eliminarSoportes')}}">
        @csrf
    </div>
</div>
<!-- Modal addManual -->
