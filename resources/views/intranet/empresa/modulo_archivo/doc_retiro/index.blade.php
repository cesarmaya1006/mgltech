...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="#">Vacaciones y licencias</a></li>
@endsection
@section('titulo_card')
<img src="{{asset('imagenes/sistema/10.png')}}" class="img-fluid" alt="..." style="max-height: 35px; width: auto;"> Vacaciones y licencias
@endsection
                    ...
                    @else
                        @if ($grupos->count()==1)
                            <select id="empresa_id" class="form-control form-control-sm"
                                    data_url="{{ route('doc_retiro.getCargarEmpleadosEmpresa') }}">
...
<div class="row">
    <div class="col-12">
        <input type="hidden" id="datosUpLaod"
                data_soportes="{{route('doc_retiro.getSoportes')}}"
                data_url="{{route('doc_retiro.setSoportes')}}"
                ruta_soporte="{{asset('documentos/soportes') . '/'}}"
                tabla ="tablaSoportes"
                tbody ="tbody_Soportes"
                tipoSoporte ="DocRetiro"
                titulo ="Ingrese el soporte de Documentos de Retiro"
                aviso ="El soporte de Documentos de Retiro">
        <input type="hidden" id="borrarSoportes" data_url="{{route('doc_retiro.eliminarSoportes')}}">
        @csrf
    </div>
</div>
<!-- Modal addManual -->
