...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="#">Procesos disciplinarios</a></li>
@endsection
@section('titulo_card')
<img src="{{asset('imagenes/sistema/7.png')}}" class="img-fluid" alt="..." style="max-height: 35px; width: auto;"> Proceso disciplinario, faltas y sanciones
@endsection
                    ...
                    @else
                        @if ($grupos->count()==1)
                            <select id="empresa_id" class="form-control form-control-sm"
                                    data_url="{{ route('proceso_discip.getCargarEmpleadosEmpresa') }}">
...
<div class="row">
    <div class="col-12">
        <input type="hidden" id="datosUpLaod"
                data_soportes="{{route('proceso_discip.getSoportes')}}"
                data_url="{{route('proceso_discip.setSoportes')}}"
                ruta_soporte="{{asset('documentos/soportes') . '/'}}"
                tabla ="tablaSoportes"
                tbody ="tbody_Soportes"
                tipoSoporte ="ProcesoDiscip"
                titulo ="Ingrese el soporte disciplinario clínica ocupacional"
                aviso ="El soporte disciplinario">
        <input type="hidden" id="borrarSoportes" data_url="{{route('proceso_discip.eliminarSoportes')}}">
        @csrf
    </div>
</div>
<!-- Modal addManual -->
