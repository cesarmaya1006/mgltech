...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="#">Situaciones laborales generales</a></li>
@endsection
@section('titulo_card')
<img src="{{asset('imagenes/sistema/5 1.png')}}" class="img-fluid" alt="..." style="max-height: 35px; width: auto;"> Situaciones laborales generales
@endsection
                    ...
                    @else
                        @if ($grupos->count()==1)
                            <select id="empresa_id" class="form-control form-control-sm"
                                    data_url="{{ route('sit_lab_gen.getCargarEmpleadosEmpresa') }}">
...
<div class="row">
    <div class="col-12">
        <input type="hidden" id="datosUpLaod"
                data_soportes="{{route('sit_lab_gen.getDocumentosContractuales')}}"
                data_url="{{route('sit_lab_gen.setDocumentosContractuales')}}"
                ruta_soporte="{{asset('documentos/soportes') . '/'}}"
                tabla ="tablaSoportes"
                tbody ="tbody_Soportes"
                tipoSoporte ="SitLabGen"
                titulo ="Ingrese la situación laboral"
                aviso ="La situación laboral">
        <input type="hidden" id="borrarSoportes" data_url="{{route('sit_lab_gen.eliminarDocumentosContractuales')}}">
        @csrf
    </div>
</div>
<!-- Modal addManual -->
